...
#ifndef __msgs_imu_hpp__
#define __msgs_imu_hpp__
...
int imu::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __double_encode_array(buf, offset + pos, maxlen - pos, &this->timestamp_valid, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int imu::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __double_decode_array(buf, offset + pos, maxlen - pos, &this->timestamp_valid, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    uint64_t hash = 0xdf76c758799b2b1aLL;
